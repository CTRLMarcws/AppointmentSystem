...
class AppointmentService {
  ...
  async findByRangeDate(initialDate, endDate) {
    const appointments = await this.appointmentRepository.findByRangeDate(
      initialDate,
      endDate
    );
    return appointments;
  ...
  async findByDocumentNumber(customerDoc) {
    const appointments = await this.appointmentRepository.findByDocumentNumber(
      customerDoc
    );
    return appointments;
  }
  ...
  async update(
    state, value, doctorName,  date, cxName, cxDocNumber, cxPhone, appointmentId
  ) {
    const oldApp = await this.appointmentRepository.findById(appointmentId);
    const newApp = await this.appointmentRepository.update(
      state, value, doctorName,  date, cxName, cxDocNumber, cxPhone, appointmentId
    );
    return newApp;
  }
